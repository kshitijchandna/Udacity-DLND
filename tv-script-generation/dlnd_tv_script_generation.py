...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    logits = tf.contrib.layers.fully_connected(rnn, vocab_size, None)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    text = np.array(int_text[:num_batches * (batch_size  * seq_length)])   
    input = text.reshape(-1, seq_length)
    text2 = np.roll(text, -1).reshape(-1, seq_length)
    output = np.zeros(shape=(num_batches, 2, batch_size, seq_length), dtype=np.int)
    for x in range(0, input.shape[0]):
        a = x % num_batches
        b = x // num_batches
        output[a,0,b,:] = input[x,:]
        output[a,1,b,:] = text2[x,:]
    return output
